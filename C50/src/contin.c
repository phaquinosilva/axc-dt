...
// @crismeinhardt, @grellert |> realmente é nessa função
...
/*   -----------------  */
{
	...
	/*  Repeatedly check next possible cut  */ // onde descobrimos os threshold
	...
	for (; i <= GEnv.Ep; i++)
	{
		...
		if (GEnv.HighVal > GEnv.LowVal)
		{
			...
			if (!GEnv.LowClass || GEnv.LowClass != GEnv.HighClass || j > GEnv.Ep)
			{
				...
				// @phaquinosilva: esse LowInfo é relevante
	...
	else
	{
		...
		if ((Bar[Att] = (ContValue)(0.5 * (GEnv.LowVal + GEnv.HighVal))) >= GEnv.HighVal)
		{
			Bar[Att] = GEnv.LowVal; // this is the threshold
		}
...
/*   -------------  */
{
	...
	for (; i <= GEnv.Ep; i++)
	{
		...
		if (GEnv.HighVal > GEnv.LowVal)
		{
			...
			if (!GEnv.LowClass || GEnv.LowClass != GEnv.HighClass || j > GEnv.Ep)
			{
				LHInfo = TotalInfo(GEnv.Freq[2], 1, MaxClass) + TotalInfo(GEnv.Freq[3], 1, MaxClass);

				SplitInfo = (GEnv.FixedSplitInfo + PartInfo(GEnv.LowCases) + PartInfo(GEnv.ApplicCases - GEnv.LowCases)) / GEnv.Cases;
...
/*   ----------------  */
{
	...
	else // @phaquinosilva: values present
	{
		GEnv.Xp = Fp;
